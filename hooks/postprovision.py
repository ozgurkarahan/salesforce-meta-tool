...
def create_agent():
    ...
    sf_oauth_connection = os.environ.get("SF_OAUTH_CONNECTION_NAME", "")
    if sf_oauth_connection:
        sf_tool_kwargs["project_connection_id"] = sf_oauth_connection
        print(f"SF OAuth connection: {sf_oauth_connection}")
    ...
    if sf_oauth_connection:
        ...
        print("\n--- Smoke test skipped (no OAuth connection) ---")
        print("Set SF credentials and re-run to enable OAuth flow.")
...
def main():
    ...
    # Step 2: Create Foundry agent (SF-only)
